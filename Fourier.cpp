...
void	CFourier::Resynthesize(struct _WaveSample_ *pWav)
{
	float		*time_signal;		// Float buffer for sample rate conversion
	float		*mdcf_line;
	byte * WaveData;
	DWORD datasize;

	datasize=pWav->data_header.dataSIZE; // + (2*MIRAGE_PAGESIZE);

	WaveData = (byte *)::GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, datasize*sizeof(byte));
	time_signal = (float *)::GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT,datasize*sizeof(float));
	mdcf_line = (float *)::GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT,datasize*sizeof(float));

	memcpy(WaveData/*+(2*MIRAGE_PAGESIZE)*/,pWav->SampleData,(int)pWav->data_header.dataSIZE);

	src_unchar_to_float_array(WaveData, time_signal, datasize);

	mdctf_init(datasize);

	mdctf(mdcf_line,time_signal);
/*	for(int n=0; n < datasize; n++)
	{
		mdcf_line[n]=mdcf_line[n]/2;
	}
*/
	imdctf(time_signal,mdcf_line);

	mdctf_free();

	src_float_to_unchar_array (time_signal/*+(2*MIRAGE_PAGESIZE)*/, (unsigned char *)&pWav->SampleData, (int)pWav->data_header.dataSIZE);
//	memcpy(pWav->SampleData,WaveData+MIRAGE_PAGESIZE,(int)pWav->data_header.dataSIZE);
}
