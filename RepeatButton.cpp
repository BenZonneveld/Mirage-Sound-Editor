...
CRepeatButton::CRepeatButton()
{
	...
	doClickSound = FALSE;	// emit a click when button is activated]
}
...
void CRepeatButton::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	NotifyParent();	// Send the click message to the parent for normal processing
...
void CRepeatButton::OnTimer(UINT nIDEvent) 
{
	...
	if(!IsOverClient()) {
		...
		if(TimerStep > 1) {
			NotifyParent();	// Send another click message to parent
		}
	}
...
void CRepeatButton::NotifyParent()
{
	// This sends the normal notification to the parent. It does not wait
	// for the parent to process the message. It just posts it to the
	// message queue and goes on its merry way.
