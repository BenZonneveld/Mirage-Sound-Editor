...
private:
	BOOL TimerID;
	int TimerStep;
	BOOL doClickSound;	// TRUE for sending out "click" sound
...
public:
	...
	void NotifyParent(void);	// Send the "click" message to parent dialog/window.
	virtual ~CRepeatButton();
	BOOL EnableSound(BOOL OnOff) {
		BOOL oldval = doClickSound;
		doClickSound = OnOff;
		return(oldval);
	}
