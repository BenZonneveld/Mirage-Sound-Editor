...
//    void unlock (void){ if (pthread_mutex_unlock (&_mutex)) abort (); }
	// pthread mutex_init
    Resampler_mutex (void) { if ((_mutex=CreateMutex(NULL,FALSE,NULL)) == NULL) abort (); }
	// pthread_mutex_destroy == CloseHandle
    ~Resampler_mutex (void) { CloseHandle(_mutex); }
	// pthread_mutex_lock == WaitForSingleObject
    void lock (void) { if (WaitForSingleObject(_mutex, INFINITE)) abort (); }
	// pthread_mutex_unlock == ReleaseMutex
    void unlock (void){ if (ReleaseMutex(_mutex)==FALSE) abort (); }
